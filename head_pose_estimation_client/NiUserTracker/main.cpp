...
Util g_util;
MovieMng g_MovieMng(g_ContextR, g_DepthGenerator, g_ImageGenerator);
SkelMng g_SkelMng;
...
void glutDisplay (void)
{
...
#ifndef USE_GLES
	//printf("depthMD.XRes:%d depthMD.YRes:%d  imageMD.XRes:%d imageMD.YRes:%d\n", depthMD.XRes(), depthMD.YRes(), imageMD.XRes(), imageMD.YRes());
	glOrtho(0, depthMD.XRes(), depthMD.YRes(), 0, -1.0, 1.0);
	glOrtho(depthMD.XRes(), imageMD.XRes(), imageMD.YRes(), 0, -1.0, 1.0);
#else
	...
	DrawMap(depthMD, sceneMD, imageMD);
...
#define SAMPLE_XML_PATH "./Config_OpenNI.xml"
...
int main(int argc, char **argv)
{
	...
	else
	{
		...
		// ファイル保存用
		nRetVal = g_ContextR.Init();
		CHECK_RC(nRetVal, "Init(for Recording)");
	}
	...
	{
		printf("Frame Sync is not supported....\n");
	}
	*/
	...
	// 各種初期化
	g_MovieMng.Initialize();
	g_SkelMng.Initialize();
