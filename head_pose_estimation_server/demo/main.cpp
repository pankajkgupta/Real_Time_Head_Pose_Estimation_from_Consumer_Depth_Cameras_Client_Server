...
///for sockets : ref - http://msdn.microsoft.com/en-us/library/windows/desktop/ms737889(v=vs.85).aspx
...
bool read_data( ){
	...
	//receive XRes
	XnUInt32 XRes = 0;
	iResult = recv(ClientSocket, (char*)&XRes, sizeof(XRes), 0);
	if (iResult > 0) {
		printf("Bytes received: %d\n", iResult);
		//int imgSize = ntohl(g_pixel_size);
		printf("XRes is: %d\n", XRes);
	}
	else if (iResult == 0)
		printf("Connection closing...\n");
	...
	//receive YRes
	XnUInt32 YRes = 0;
	iResult = recv(ClientSocket, (char*)&YRes, sizeof(YRes), 0);
	if (iResult > 0) {
		printf("Bytes received: %d\n", iResult);
		//int imgSize = ntohl(g_pixel_size);
		printf("YRes is: %d\n", YRes);
	...
	XnUInt8 *datap = new XnUInt8[recvImageSize];
	
	pDepth = new XnDepthPixel[recvImageSize];
	//receive depth data
	iResult=0;
	int totalDataReceived=0;
	do{
		printf("receiving depth data\n");
		printf("size of datap: %d\n", sizeof(&datap));
		iResult = recv(ClientSocket, (char*)datap+totalDataReceived, recvImageSize-totalDataReceived, 0);
		totalDataReceived += iResult;
		
		if (iResult > 0) {
			printf("Bytes received: %d\n", iResult);
			printf("total image size: %d\n", recvImageSize);
			//int imgSize = ntohl(g_pixel_size);
			//printf("pixel size in mm: %d\n", g_pixel_size);
	...
	}while(totalDataReceived<recvImageSize);
	memcpy(pDepth, datap, totalDataReceived);
	
	printf("***********************received depth data %d\n", totalDataReceived);
	delete [] datap;
	
	//this code is for socket cleanup -- will be used later
	if(0)
	{
...
int main(int argc, char* argv[])
{
	...
	int ret = initialize();
	cout << "initialize() return value" << ret << endl;
	// initialize GLUT
